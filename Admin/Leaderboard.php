...
// Top 3 students ordered by number of sit-ins, then by earliest sit-in and latest sit-out time
$top_query = "SELECT 
    s.idNo, s.firstname, s.lastname, s.points, s.sessions, si.sitin_time, si.sit_out_time,
    COUNT(si.id) AS sit_in_count, 
    SEC_TO_TIME(SUM(TIMESTAMPDIFF(SECOND, si.sitin_time, si.sit_out_time))) AS total_duration,
    MIN(si.sitin_time) AS first_sit_in,
    MAX(si.sit_out_time) AS last_sit_out
    FROM student s 
    LEFT JOIN sit_in si ON s.idNo = si.idno 
    GROUP BY s.idNo 
    ORDER BY sit_in_count DESC, first_sit_in DESC, last_sit_out DESC
    LIMIT 3";
$top_result = $conn->query($top_query);

// Full leaderboard ordered by number of sit-ins, then by earliest sit-in and latest sit-out time
$all_query = "SELECT 
    s.idNo, s.firstname, s.lastname, s.points, s.sessions, si.sitin_time, si.sit_out_time,
    COUNT(si.id) AS sit_in_count, 
    SEC_TO_TIME(SUM(TIMESTAMPDIFF(SECOND, si.sitin_time, si.sit_out_time))) AS total_duration,
    ...
    <table>
        ...
        ?>
            <tr>
                ...
                <td><?= $row['points'] ?></td>
                <td><?= $row['sessions'] ?></td>
                <td><?= $row['sitin_time'] ?></td>
                <td><?= $row['sit_out_time'] ?></td>
            </tr>
        <?php endwhile; ?>
        </tbody>
    </table>
